...
import { aws_greengrassv2 as gg2 } from 'aws-cdk-lib';
...
export class SampleLoggerComponent extends base.BaseConstruct {
  ...
  constructor(scope: Construct, id: string, props: ConstructProps) {
    ...
    const recipe: any = this.createRecipe(props.bucket, props.compConfig, iotRuleTopic);

    const ggComponent = new gg2.CfnComponentVersion(this, `${this.compName}Comp`, {
      inlineRecipe: JSON.stringify(recipe)
    });
      ...
      "Manifests": [
        {
          ...
          "Lifecycle": {
            "Setenv": {
              "COMP_NAME_VERSION": `${this.compName}:${compVersion}`,
              "RULE_TOPIC": ruleTopic,
              "SLEEP_TIME": '22',
              "FUNCTION_VERSION": `${this.compName}:${compVersion}`,
            },
            "Install": {
